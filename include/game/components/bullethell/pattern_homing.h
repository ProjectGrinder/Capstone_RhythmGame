...
namespace Game::BulletHell
{
    struct Homing
    {
        ...
        {}
        explicit Homing(const Physics::Position targetPosition, const float delay, const float expire_time, float strength) :
            targetPlayer(false), targetPosition(targetPosition),delay(delay), expire_time(expire_time), strength(strength)
        {}
    };
}