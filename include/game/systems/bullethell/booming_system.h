    ...
    void boomer_system([[maybe_unused]] T &syscall, System::ECS::Query<Booming , Particle,Delay, Physics::Scale, Render::Material>& query, System::ECS::Query<Battle::BattleState> &query2)
    {
        ...
        for (auto &[id, comps] : query)
        {
            ...
            if (!boomer.is_activated)
            {
                ...
                {
                    scl.scaleX += static_cast<float>(Battle::get_delta_time()/1000000 * boomer.max_size / boomer.boom_frame);
                    scl.scaleY += static_cast<float>(Battle::get_delta_time()/1000000 * boomer.max_size/ boomer.boom_frame);
