...
namespace Game::Rhythm
{
            ...
            [[maybe_unused]] System::ECS::Query<KeyInput> &input_query)
    {
        ...
        if (note_id < 0 || note_comp == nullptr)
            return;
        ...
        if (note_comp->get<TimingEnd>().timing_end == 0)
        {
            // it's a tap note
            auto time_diff =
                    note_comp->get<Timing>().timing - battle_query.front().get<Battle::BattleState>().clock_time;
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    auto time_diff_end = note_comp->get<TimingEnd>().timing_end -
                                         battle_query.front().get<Battle::BattleState>().clock_time;
