...
namespace Game::Overview
{
    ...
    void detect_overlap_interactable([[maybe_unused]] T &syscall,
                        System::ECS::Query<BulletHell::Input> &input_query,
                         System::ECS::Query<Interactable, Physics::Position, Physics::Scale, Physics::CircularCollider> &interactable_query,
        ...
        for (auto &[pid, player_comps] : player_query)
        {
            ...
            for (auto &[id, comps] : interactable_query)
            {
                ...
                if (distance_squared <= trigger_radius*trigger_radius)
                {
                    ...
                    if (input.z) comps.get<Interactable>().event_occupied = true;
                }
            }
        }
    }
