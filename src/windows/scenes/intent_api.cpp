...
HRESULT intent_storage_init(IntentStorageHandler *handler)
{
    ...
    System::IntentStorage *manager = nullptr;
    void *manager_alloc = nullptr;

    manager_alloc = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(System::IntentStorage));
    ...
    manager = new (manager_alloc) System::IntentStorage();
...
void intent_storage_cleanup(IntentStorageHandler *api)
{
    ...
    auto *manager = static_cast<System::IntentStorage *>(*api);
