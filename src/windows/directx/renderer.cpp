...
void render_sprite(RenderingEnvironment *env, const SpriteRenderObject *obj)
{
    ...
    ID3D11Buffer *current_vb = obj->vertex_buffer.Get();
    if (env->vertex_buffer != current_vb || env->stride != obj->stride)
    {
        env->vertex_buffer = current_vb;
        env->stride = obj->stride;
        env->offset = 0;
    ...
    ID3D11ShaderResourceView *srv = obj->texture_view.Get();
    env->context->PSSetShaderResources(0, 1, &srv);

    ID3D11SamplerState *sampler = obj->sampler_state.Get();
    env->context->PSSetSamplers(0, 1, &sampler);

    env->context->DrawIndexed(obj->count.index_count, obj->offset, obj->vertex_base);

    ID3D11ShaderResourceView *null_srv = nullptr;
    env->context->PSSetShaderResources(0, 1, &null_srv);
}