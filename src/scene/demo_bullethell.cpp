...
Game::Battle::BulletLoader Scene::create_bullet_data2()
{
    ...
    for (int i=0;i<500;i++)
    {
        for (int j=0;j<8;j++)
        {
            loader.CreateBullet(1000 + i*100, BulletData(0, 0, 100, (static_cast<float>(j) *45) + i*6,50,-45.f, 0, (i*8 + j)%152));
        }
    }
...
Game::Battle::BulletLoader Scene::create_bullet_collision_test()
{
    ...
    for (int i=0;i<500;i++)
    {
        for (int j=0;j<8;j++)
        {
            loader.CreateBullet(1000 + i*250 + j*50, BulletData(0, 0, 100, (static_cast<float>(j) *45) + i*6,50,-45.f, 0, (i*8 + j)%158));
        }
    }
